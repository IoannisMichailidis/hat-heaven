...
const protect = asyncHandler(async (req, res, next) => {
    ...
    if (token) {
        try {
            ...
            req.user = await User.findById(decoded.userId).select('-password'); // exclude the password and add the result to the request object as user
            next(); // move to the next piece of middleware
        } catch (error) {
            console.log(error);
            res.status(401);
            throw new Error('Not authorized, token failed'); // throw error using the custom error handler (errorHandler which is located at the end of routers int the serer.js)
        }
    } else {
        res.status(401);
