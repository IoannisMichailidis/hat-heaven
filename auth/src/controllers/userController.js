...
} from "../events/publishers/publishers.js";
...
const loginUser = asyncHandler(async (req, res) => {
  ...
  if (user && (await user.matchPassword(password))) {
    // matchPassword method created in the userModel.js
...
const updateUserProfile = asyncHandler(async (req, res) => {
  ...
  if (user) {
    ...
    if (req.body.password) {
      // Update password if there is req.body
...
const deleteUser = asyncHandler(async (req, res) => {
  ...
  if (user) {
    ...
    // Publish an event/channel/subject to NATS streaming server
    const publisher = new UserDeletedPublisher(natsWrapper.client);
    // that process is async. Handled that process manually in the Publisher
    await publisher.publish({
      id: user._id,
    });
...
const updateUser = asyncHandler(async (req, res) => {
  ...
  if (user) {
    ...
    // Publ	ish an event/channel/subject to NATS streaming server
    const publisher = new UserUpdatedPublisher(natsWrapper.client);
    // that process is async. Handled that process manually in the Publisher
    await publisher.publish({
      id: user._id,
